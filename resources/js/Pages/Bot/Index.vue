<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="p-4 bg-white rounded-lg shadow-xs">
            ...
            <form @submit.prevent="setWebhook" class="w-1/2">
                <div class="mt-4">
                    <InputLabel for="url" value="Url"/>
                    <TextInput id="url" type="text" class="block mt-1 w-full" v-model="webhook_form.webhook_url"/>
                    <InputError class="mt-2" :message="webhook_form.errors.webhook_url"/>
                </div>
            ...
            <div class="mt-2">
                <PrimaryButton class="block mt-4 w-full" @click="deleteWebhook"
                               :class="{ 'opacity-25': delete_webhook_form.processing }"
                               :disabled="delete_webhook_form.processing">
                    Delete Webhook
                </PrimaryButton>
            </div>
        </div>
...
const webhook_form = useForm({
    _method: 'POST',
    webhook_url: ''
});

const delete_webhook_form = useForm({
    _method: 'POST'
});

const deleteWebhook = () => {
    delete_webhook_form.post(route('bot.delete_webhook'));
};

const setWebhook = () => {
    webhook_form.post(route('bot.set_webhook'));
};
